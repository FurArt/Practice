...
function savilerow_scripts() {
  ...
  if( is_woocommerce_activated() ){
    if( is_product_category() ){
      ...
      wp_enqueue_script( 'savilerow-woo-script', get_template_directory_uri() . '/js/savilerow.woo.script.js', array( 'jquery', 'jquery-matchHeight' ), '1.0', true );
    }
  }
...
function savile_row_welcome_redirect(){
  global $pagenow;
  if ( is_admin() && 'themes.php' == $pagenow && isset( $_GET['activated'] ) ) {
  	wp_redirect( admin_url( 'themes.php?page=savilerow-welcome' ) );
  }
}
add_action('after_setup_theme', 'savile_row_welcome_redirect');
 ...
 */
 function savilerow_get_pro_link( $content ) {
	return esc_url( 'https://www.templateexpress.com/savilerow-pro-theme' );
}
...
/**
 * Define default WooCommerce image sizes
 */
